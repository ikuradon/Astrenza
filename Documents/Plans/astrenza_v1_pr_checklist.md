...
- [ ] First minimal Root call-site integration follows `Documents/Plans/timeline_home_limited_wiring_plan.md`: no-op `TimelineHomeRootRoutePreflight` call only, default legacy explicit, collection view decision may be recorded locally but `TimelineCollectionViewController` and collection view `TimelineSurface` construction from Root remain closed, no same-session fallback after visible mutation, and no production replacement of legacy Home.
- [ ] First minimal Root call-site integration calls `TimelineHomeRootRoutePreflight.invoke(_:)` or a tiny wrapper around it, not `TimelineHomeRouteHost.decide`, `TimelineHomeRouteAdapter.decide`, or `TimelineHomeRouteIntegrationSkeleton.select` directly from Root.
- [ ] First minimal Root call-site integration injects launch arguments, debug override, `createdAtMS`, and dependency readiness into `TimelineHomeRootRoutePreflight`; the Root call site does not read `ProcessInfo.processInfo.arguments` directly unless a tiny adapter reads it and injects it into the preflight input.
- [ ] First minimal Root call-site integration attaches a local route decision artifact for debug review only: `selectedRoute`, fallback reason or none, dependency readiness, side-effect sentinel all false, `requiresNetworkWork == false`, `requiresDBWrite == false`, `readMarkerChanged == false`, and `preventsDualMutation == true` are visible without route construction, relay startup, network work, DB writes, external upload, telemetry, analytics, remote logging, raw private content, raw event JSON, `nsec`, Keychain data, signing payloads, or bearer tokens.
- [ ] First minimal Root call-site integration proves `TimelineHomeRootRouteCallSiteTests` executed non-zero Swift Testing tests, including `default_root_route_preflight_keeps_legacy_home`, `root_route_preflight_does_not_construct_collection_view`, `root_route_preflight_does_not_construct_nostr_store`, `root_route_preflight_records_local_diagnostics`, `collection_view_flag_records_decision_but_does_not_enable_by_default`, `unknown_flag_falls_back_to_legacy`, `read_marker_not_advanced_by_root_preflight`, `no_network_started_by_root_preflight`, `no_db_write_by_root_preflight`, `dataSourceApply_coordinator_only`, and `selected_swift_testing_suites_non_zero`.
- [ ] Actual collection view route construction remains closed until no-op Root preflight call-site tests pass, default legacy behavior is unchanged in app-hosted tests, the route diagnostics artifact is generated locally, offscreen controller smoke and snapshot coordinator harness remain green, no startup network logs appear, `networkWaitedBeforeInteractiveScrollMS == 0`, `readMarkerChanged == false`, and old/new dual mutation prevention remains true.
- [ ] First minimal Root call-site integration keeps `NostrHomeTimelineStore`, legacy SwiftUI Home, root shell, startup splash, schema, migration, DB write paths, relay/network/resolver startup, real `ResolveCoordinator`, `resolve_jobs`, `TimelinePlaceholderRow`, project/dependency files, and `.github` unchanged unless a later approved scope explicitly opens them.
