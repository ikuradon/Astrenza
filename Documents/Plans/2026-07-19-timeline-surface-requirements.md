...
実装候補は`UICollectionView` + system self-sizing layout + `UIHostingConfiguration`とする。
custom `UICollectionViewLayout`を使用する場合は、SwiftUIの事後測定を禁止し、表示前に決定できる
native/deterministic Row rendererが必要になるため、今回の第一選択にはしない。
...
4. `UICollectionView`のsystem self-sizingを唯一のgeometry authorityとして組み直す。
5. `TimelineRowPresentation`を導入し、paint-only updateとgeometry updateを分離する。
