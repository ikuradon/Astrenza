...
Status: Phase 0/1 complete; Phase 5 scaffold complete; Phase 6 fixture contracts mostly complete; Phase 4 read-only DB/read-state bridge boundary exists for planning; the current Home route work can no-op Root preflight, record local diagnostics, snapshot Root-visible decisions, read those snapshots locally, read construction plans/harness results, read the route/construction artifact chain locally, and construct the flagged collection view route only as an offscreen/non-rendered path. Activation/render switching remains closed.
Updated: 2026-07-02
...
Status: Scaffold complete for offline tests. Production Home replacement remains blocked. The future Home wiring strategy is documented in `Documents/Plans/timeline_home_wiring_adr.md`, and the collection view route activation gates are defined in `Documents/Plans/timeline_home_limited_wiring_plan.md`; both remain docs-only for activation and do not authorize production default rollout.
...
- Future activation/render switch is later than construction. Activation means Root/Home may render the already constructed collection view path. It requires explicit `--timeline-engine=collectionView`, all construction gates still clean, default without the flag remaining legacy, manual rollback remaining legacy, route diagnostics / construction artifact chain recording, Root shell first paint preserved, Timeline restore gate scoped to the Timeline area only, `networkWaitedBeforeInteractiveScrollMS == 0`, and `readMarkerChanged == false`.
...
Keep collection view route activation/render switching closed while preparing the future activation slice. The next implementation task is titled `test: activate TimelineHome collectionView route behind flag`, and must first add RED-first `TimelineHomeCollectionViewRouteActivationTests`.

That future task may switch the rendered route only behind explicit `--timeline-engine=collectionView` and readiness gates, use the already constructed flagged collection view path, keep route diagnostics / construction artifact chain recording, preserve Root shell first paint, keep the Timeline restore gate scoped to the Timeline area only, keep `networkWaitedBeforeInteractiveScrollMS == 0`, and keep `readMarkerChanged == false`. It must require flagged construction review PASS, offscreen/no-window smoke PASS, initial restore snapshot coordinator harness PASS, artifact chain consumer PASS, startup network grep with no matches, privacy guard PASS, coordinator-only `dataSource.apply`, no extra `NostrHomeTimelineStore`, no DB write/read marker mutation, and a Root body decision snapshot that explicitly permits activation scope.

Required future tests are `TimelineHomeCollectionViewRouteActivationTests`, `activation_requires_explicit_flag`, `activation_requires_all_construction_gates`, `default_legacy_rendering_remains_default`, `activation_does_not_start_network_before_interactive_scroll`, `activation_does_not_advance_read_marker`, `activation_does_not_write_db`, `activation_does_not_call_dataSourceApply_from_Root`, `activation_does_not_construct_extra_NostrHomeTimelineStore`, `activation_records_route_and_construction_artifacts`, `activation_uses_timeline_area_restore_gate_only`, `activation_keeps_root_shell_first_paint`, `activation_rollback_returns_to_legacy`, and `selected_swift_testing_suites_non_zero`.

The activation slice must avoid defaulting to collection view, avoid removing or bypassing `NostrHomeTimelineStore`, avoid starting relay/network before first interactive scroll, avoid DB write/read marker/`feed_read_state`/`pending_new` mutation, avoid Root `dataSource.apply`, avoid splash/root shell behavior changes, avoid schema/migration/GitHub Actions/dependency changes, and avoid same-session double mutation between legacy and collection view paths. If any activation gate fails, do not render the collection view route; record the blocked artifact and keep legacy. Rollback and manual fallback remain legacy.
