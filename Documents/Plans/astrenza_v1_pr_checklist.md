...
- [ ] Production Root body render switching remains closed in the current docs-only slice: current behavior is legacy rendering plus the flagged offscreen/non-rendered construction result, activation switch helper result, Root body activation wiring gate result, and local diagnostics; debug override `.collectionView` cannot bypass the launch flag, and default legacy remains required.
...
- [ ] Root body render switching remains later than construction, activation switch helper, Root activation decision chain, and Root body activation wiring gate: the Root body wiring gate must still report `renderedRouteDecision == legacy`, `productionRootBodyChanged == false`, `collectionViewRenderingActivated == false`, manual rollback and manual fallback must be `legacy`, and if any gate fails, do not render collection view; record the artifact and keep legacy.
- [ ] Future Root body render switch implementation is titled `test: wire TimelineHome collectionView route into Root body behind flag`; Root body render switch means `AstrenzaRootView.body` may choose collection view Home only behind explicit `--timeline-engine=collectionView` and a clean wiring gate, keeps default without the flag legacy, keeps rollback/manual fallback legacy, preserves Root shell first paint, keeps the Timeline restore gate Timeline-area-only, and must not introduce startup network, DB writes, read-marker mutation, or Root `dataSource.apply`.
- [ ] Future Root body render switch allowed scope is limited to touching `AstrenzaRootView.body` only for an explicit flagged route branch, using injected mode / preflight / wiring gate output, rendering the collection view route only when `wiringAllowed == true`, recording the local diagnostics/artifact decision, keeping legacy route as fallback and rollback, preserving root shell first paint, and keeping same-session double mutation prevented.
- [ ] Future Root body render switch forbidden scope includes defaulting to collection view, removing or bypassing `NostrHomeTimelineStore`, relay/network/resolver startup before first interactive scroll, DB write/read marker/`feed_read_state`/`pending_new` mutation, `dataSource.apply` from Root, splash/root shell behavior changes, GitHub Actions/dependency/schema/migration changes, external telemetry/upload/export paths, opening the `ResolveCoordinator` actor, and media/OGP resolver wiring.
- [ ] Before future Root body render switch opens, required gates are PASS: flagged construction, activation switch helper, activation readiness, activation readiness consumer, activation artifact chain consumer, Root activation preflight, Root activation decision snapshot chain, Root activation decision snapshot chain consumer, Root body activation wiring gate, Root body activation wiring gate consumer, `TimelineCollectionViewControllerSmokeTests`, `TimelineInitialRestoreSnapshotCoordinatorHarnessTests`, startup-network grep no matches, coordinator-only `dataSource.apply`, no extra `NostrHomeTimelineStore`, no DB write/read marker/`feed_read_state`/`pending_new` mutation, privacy guard/self-test, and selected Swift Testing suites non-zero.
- [ ] Future Root body render switch tests exist before implementation opens Root body switching: `TimelineHomeRootBodyRenderSwitchTests`, `root_body_render_switch_requires_explicit_flag`, `root_body_render_switch_requires_clean_wiring_gate`, `default_without_flag_renders_legacy`, `dirty_wiring_gate_renders_legacy`, `clean_flagged_wiring_renders_collectionView`, `render_switch_preserves_root_shell_first_paint`, `render_switch_uses_timeline_area_restore_gate_only`, `render_switch_does_not_start_network_before_interactive_scroll`, `render_switch_does_not_write_db`, `render_switch_does_not_advance_read_marker`, `render_switch_does_not_call_dataSourceApply_from_Root`, `render_switch_does_not_construct_extra_NostrHomeTimelineStore`, `render_switch_prevents_same_session_double_mutation`, `rollback_returns_to_legacy`, and `selected_swift_testing_suites_non_zero`.
