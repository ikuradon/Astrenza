...
Status: Scaffold complete for offline tests. Production Home replacement remains blocked. The future Home wiring strategy is documented in `Documents/Plans/timeline_home_wiring_adr.md`, and the Root body render switch final gate is defined in `Documents/Plans/timeline_home_limited_wiring_plan.md`; both remain docs-only for `AstrenzaRootView.body` render switching and do not authorize production default rollout.
...
Keep production Root body render switching closed while preparing the future Root body render switch slice. The next implementation task is titled `test: wire TimelineHome collectionView route into Root body behind flag`, and must first add RED-first `TimelineHomeRootBodyRenderSwitchTests`.

That future task may touch `AstrenzaRootView.body` only for an explicit flagged route branch, use injected mode / preflight / wiring gate output, render the collection view route only when `wiringAllowed == true`, record the local diagnostics/artifact decision, keep legacy route as fallback and rollback, preserve Root shell first paint, keep the Timeline restore gate scoped to the Timeline area only, and keep same-session double mutation prevented.

Required preconditions are flagged construction PASS, activation switch helper PASS, activation readiness PASS, activation readiness consumer PASS, activation artifact chain consumer PASS, Root activation preflight PASS, Root activation decision snapshot chain PASS, Root activation decision snapshot chain consumer PASS, Root body activation wiring gate PASS, Root body activation wiring gate consumer PASS, `TimelineCollectionViewControllerSmokeTests` PASS, `TimelineInitialRestoreSnapshotCoordinatorHarnessTests` PASS, startup-network grep with no matches, coordinator-only `dataSource.apply`, no extra `NostrHomeTimelineStore`, no DB write/read marker/`feed_read_state`/`pending_new` mutation, privacy guard/self-test PASS, and selected Swift Testing suites non-zero.

Required future tests are `TimelineHomeRootBodyRenderSwitchTests`, `root_body_render_switch_requires_explicit_flag`, `root_body_render_switch_requires_clean_wiring_gate`, `default_without_flag_renders_legacy`, `dirty_wiring_gate_renders_legacy`, `clean_flagged_wiring_renders_collectionView`, `render_switch_preserves_root_shell_first_paint`, `render_switch_uses_timeline_area_restore_gate_only`, `render_switch_does_not_start_network_before_interactive_scroll`, `render_switch_does_not_write_db`, `render_switch_does_not_advance_read_marker`, `render_switch_does_not_call_dataSourceApply_from_Root`, `render_switch_does_not_construct_extra_NostrHomeTimelineStore`, `render_switch_prevents_same_session_double_mutation`, `rollback_returns_to_legacy`, and `selected_swift_testing_suites_non_zero`.

The Root body render switch slice must avoid defaulting to collection view, avoid removing or bypassing `NostrHomeTimelineStore`, avoid starting relay/network/resolver work before first interactive scroll, avoid DB write/read marker/`feed_read_state`/`pending_new` mutation, avoid Root `dataSource.apply`, avoid splash/root shell behavior changes, avoid schema/migration/GitHub Actions/dependency changes, avoid external telemetry/upload/export paths, avoid opening the `ResolveCoordinator` actor or media/OGP resolver wiring, and avoid same-session double mutation between legacy and collection view paths. If any wiring gate fails, do not render the collection view route; record the blocked artifact and keep legacy. Rollback and manual fallback remain legacy.
