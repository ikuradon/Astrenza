...
- TimelineHome route and launch-restore contracts now exist for planning: `TimelineHomeEngineMode`, `TimelineHomeRouteAdapter`, `TimelineHomeRouteIntegrationSkeleton`, `TimelineHomeRouteHost`, `TimelineHomeRouteDiagnostics`, `TimelineHomeRouteDiagnosticsSink`, `TimelineHomeRootRouteGuard`, `TimelineHomeRootRoutePreflight`, `TimelineHomeRootRouteCallSite`, `TimelineHomeRootRouteDecisionSnapshot`, `TimelineHomeRootRouteDecisionSnapshotConsumer`, `TimelineHomeLaunchRestoreContract`, `TimelineSurfaceDependencyContainer`, `TimelineInitialRestoreUseCase`, and the snapshot coordinator harness. `AstrenzaRootView` already invokes the no-op Root preflight while keeping legacy Home visible, local route diagnostics can be recorded and read, and `Documents/Plans/timeline_home_limited_wiring_plan.md` now defines the exact gates for opening a future collection view route construction task. Construction remains closed now.
...
- Future construction cannot open until Root no-op preflight, route diagnostics sink injection, root decision snapshot, snapshot consumer, offscreen controller smoke, restore harness, non-zero selected Swift Testing suites, startup-network grep, artifact privacy guard, `networkWaitedBeforeInteractiveScrollMS == 0`, `readMarkerChanged == false`, `requiresNetworkWork == false`, `requiresDBWrite == false`, coordinator-only `dataSource.apply`, and no extra collectionView-path `NostrHomeTimelineStore` construction all pass.
...
Keep collection view route construction closed while preparing the future construction slice. The next implementation task that could open construction should be titled `test: construct TimelineHome collectionView route behind flag` or equivalent, and must first add RED-first `TimelineHomeRouteConstructionReadinessTests` and `TimelineHomeCollectionViewRouteConstructionTests`.

That future task may only construct read-only/offline `TimelineSurfaceDependencyContainer` dependencies plus a `TimelineSurface` or `TimelineCollectionViewController` behind explicit `--timeline-engine=collectionView` and readiness gates. It must keep `renderedRoute == legacy` unless a later activation task opens rendering; avoid production Home/root/splash replacement; avoid defaulting to collection view; avoid removing or bypassing `NostrHomeTimelineStore`; keep relay/network/resolver/DB write/read marker/`feed_read_state`/`feed_items.pending_new`/`resolve_jobs`/`dataSource.apply` scope closed; prove no extra `NostrHomeTimelineStore` construction in the collection view route construction path; pass startup-network grep with no matches; pass artifact privacy guards; and prove every selected app suite executes a non-zero Swift Testing count. Route activation/rendering switch is later than construction and requires a separate explicit scope.
