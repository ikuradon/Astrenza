...
Status: Phase 0/1 complete; Phase 5 scaffold complete; Phase 6 fixture contracts mostly complete; Phase 4 read-only DB/read-state bridge boundary exists for planning; the current Home route work can no-op Root preflight, record local diagnostics, snapshot Root-visible decisions, read those snapshots locally, read construction plans/harness results, read the route/construction artifact chain locally, construct the flagged collection view route only as an offscreen/non-rendered path, evaluate the activation switch helper, read the Root body activation wiring gate, switch the Root body only behind explicit `--timeline-engine=collectionView` plus a clean wiring gate, compose the flagged collection view restore plan, and run the privacy-safe startup smoke gate. Default/no-flag startup remains legacy.
Updated: 2026-07-06
...
- Phase 5 TimelineEngine scaffold is complete enough for offline scaffold tests: `TimelineSurface`, `TimelineCollectionViewController`, diffable data source, snapshot coordinator, position recorder, visible range tracker, prefetch coordinator, diagnostics recorder, construction readiness, construction plan consumer, offscreen harness result consumer, and construction artifact chain consumer exist under `Astrenza/Sources/AstrenzaApp/TimelineEngine`. It is not wired into production Home.
...
- TimelineHome route and launch-restore contracts now exist for planning: `TimelineHomeEngineMode`, `TimelineHomeRouteAdapter`, `TimelineHomeRouteIntegrationSkeleton`, `TimelineHomeRouteHost`, `TimelineHomeRouteDiagnostics`, `TimelineHomeRouteDiagnosticsSink`, `TimelineHomeRootRouteGuard`, `TimelineHomeRootRoutePreflight`, `TimelineHomeRootRouteCallSite`, `TimelineHomeRootRouteDecisionSnapshot`, `TimelineHomeRootRouteDecisionSnapshotConsumer`, `TimelineHomeRouteConstructionReadiness`, `TimelineHomeRouteConstructionPlanConsumer`, `TimelineHomeRouteConstructionReadinessConsumer`, `TimelineHomeOffscreenConstructionHarnessResultConsumer`, `TimelineHomeConstructionArtifactChainConsumer`, `TimelineHomeCollectionViewRouteActivationReadiness`, `TimelineHomeCollectionViewRouteActivationReadinessConsumer`, `TimelineHomeActivationArtifactChainConsumer`, `TimelineHomeRootActivationPreflight`, `TimelineHomeRootActivationDecisionSnapshotChain`, `TimelineHomeRootActivationDecisionSnapshotChainConsumer`, `TimelineHomeCollectionViewRouteActivationSwitch`, `TimelineHomeRootBodyActivationWiringGate`, `TimelineHomeRootBodyActivationWiringGateConsumer`, `TimelineHomeRootBodyRenderSwitch`, `TimelineHomeCollectionViewRouteRestore`, `TimelineHomeCollectionViewStartupSmoke`, `TimelineHomeLaunchRestoreContract`, `TimelineHomeCollectionViewRouteConstruction`, `TimelineSurfaceDependencyContainer`, `TimelineInitialRestoreUseCase`, and the snapshot coordinator harness. `AstrenzaRootView` already invokes the no-op Root preflight while keeping legacy Home as the default, local route diagnostics can be recorded and read, `f163ed0` added flagged collection view route construction as offscreen/non-rendered only, `18be791` added the pure activation switch helper, `5611495` added the Root body activation wiring gate, `6de53d0` added the Root body activation wiring gate consumer, `491b7e9` added the explicit-flag Root body render switch, and `4b913a3` repaired the startup smoke privacy schema. `Documents/Plans/timeline_home_limited_wiring_plan.md` now defines the startup smoke acceptance baseline for future TimelineHome startup gates.
...
Status: Scaffold complete for offline tests. Production Home default replacement remains blocked. The Home wiring strategy is documented in `Documents/Plans/timeline_home_wiring_adr.md`, and the startup smoke acceptance baseline is defined in `Documents/Plans/timeline_home_limited_wiring_plan.md`; both preserve default legacy startup and do not authorize production default rollout.
...
- Root call-site diagnostics remains no-op, defaults to legacy, records local in-memory route decision artifacts, supports Root-visible decision snapshots and local consumers, avoids collection view route construction, avoids same-session dual mutation, and keeps production Home/root/splash behavior unchanged.
- Collection view route construction is defined as Root/Home constructing a `TimelineSurface` or `TimelineCollectionViewController` dependency path behind `--timeline-engine=collectionView` and readiness gates. It is not default rendering, route activation, or a rendering switch.
- Future construction cannot open until Root no-op preflight, route diagnostics sink injection, root decision snapshot, snapshot consumer, construction readiness, construction plan consumer, offscreen harness, offscreen harness result consumer, artifact chain consumer, `TimelineCollectionViewControllerSmokeTests`, `TimelineInitialRestoreSnapshotCoordinatorHarnessTests`, non-zero selected Swift Testing suites, startup-network grep, artifact privacy guard/self-test, `networkWaitedBeforeInteractiveScrollMS == 0`, `readMarkerChanged == false`, `requiresNetworkWork == false`, `requiresDBWrite == false`, coordinator-only `dataSource.apply`, and no extra collectionView-path `NostrHomeTimelineStore` construction all pass.
- Root body render switch is later than construction, activation switch helper, Root activation decision chain, and Root body activation wiring gate. It may render the already constructed collection view route only behind explicit `--timeline-engine=collectionView` and a clean wiring gate. It requires default without the flag remaining legacy, manual rollback remaining legacy, route diagnostics / artifact-chain recording, Root shell first paint preserved, Timeline restore gate scoped to the Timeline area only, `networkWaitedBeforeInteractiveScrollMS == 0`, `readMarkerChanged == false`, no startup network, no DB write/read marker/`feed_read_state`/`pending_new` mutation, coordinator-only `dataSource.apply`, no extra `NostrHomeTimelineStore`, privacy guard/self-test PASS, and selected Swift Testing suites non-zero.
- Startup smoke acceptance requires a fixed result bundle generated for `TimelineHomeFlaggedCollectionViewStartupSmokeTests`, the result bundle path in the report, startup-network scan output for that fixed bundle, privacy encoded JSON checks, selected Swift Testing suite counts, and no selected suite with 0 Swift Testing tests. XCTest wrapper `Executed 0 tests` alone is not evidence.
- Startup smoke artifacts must not encode raw result-bundle lines, raw excerpts, or raw `launchArguments`. Pattern hits may include only `patternKind`, `tokenID`, `lineNumber` or occurrence index, and a fixed `redactedSummary`; launch arguments must be normalized to known flags, `requestedEngineMode`, `unknownArgumentCount`, and `redactedUnknownArguments`.
- Startup smoke forbidden-fragment checks must include `nsec`, `secret`, `privateKey`, `private_key`, `raw_json`, `rawEvent`, `raw_event`, `mnemonic`, `keychain`, `nostr secret`, relay URL, pubkey, event id, and private message content phrase.
- Startup-network scan tokens are `LocalDataTask`, `ATS failure`, `nw_`, `WebSocket`, `URLSessionWebSocketTask`, `wss://`, `setDefaultRelays`, and relay connection attempts. Plain `URLSession` duplicate-class warnings must be separated from actual startup attempts.
- Root body render switch and startup smoke must not default to collection view, remove or bypass `NostrHomeTimelineStore`, start relay/network/resolver work before first interactive scroll, write DB/read marker/`feed_read_state`/`pending_new` state, call `dataSource.apply` from Root, change splash/root shell behavior, change schema/migrations/GitHub Actions/dependencies, add external telemetry/upload/export paths, open the `ResolveCoordinator` actor, wire media/OGP resolver runtime, or allow same-session double mutation between legacy and collection view paths.
...
Use the startup smoke acceptance baseline as the hard gate for future TimelineHome startup work. The next implementation task should keep default/no-flag startup legacy, require explicit `--timeline-engine=collectionView`, require a clean Root body wiring gate, reject dirty fixed result bundle scans, reject stale restore input, keep rollback/manual fallback legacy, and preserve no DB write/read-marker/`feed_read_state`/`pending_new` mutation, no Root `dataSource.apply`, no startup network/resolver work, and no telemetry/export/upload path.

Every future startup gate report must include the fixed result bundle path, startup-network scan output, privacy encoded JSON check, selected suite counts, and explicit confirmation that no selected Swift Testing suite executed 0 tests.

The local review / release checklist packet for TimelineHome startup gates must attach the fixed startup smoke result bundle path, selected app suite result bundle path, startup-network scan output, privacy scan output, encoded diagnostics attachment summary, encoded `TimelineHomeStartupSmokeEvidenceBundle` summary, encoded `TimelineHomeStartupSmokeLocalGateReport` summary, selected suite counts, zero selected suite count, `AstrenzaCore` total test count when run, `TimelineRepositoryStore` suite count when run, pushed git SHA, `HEAD == origin/main`, and clean worktree confirmation. This packet is evidence only and does not authorize source/test changes, CI or `.github` changes, production source changes, SQL/migration/dependency changes, upload/export telemetry, Root/Home/splash behavior changes, DB writes, read marker mutation, `feed_read_state` / `pending_new` mutation, startup network/resolver work, Root-owned `dataSource.apply`, or `ResolveCoordinator` actor work.

The local gate report pass criteria are strict: `usedCollectionViewFlag == true`, `selectedRoute == collectionView`, `renderedRoute == collectionView`, clean Root body wiring gate evidence, `startupNetworkScanStatus == pass` or `.clean`, `privacyScanStatus == pass` or `.passed`, `selectedSwiftTestingSuitesNonZero == true`, zero selected suite count, every side-effect sentinel clean, no raw bundle lines, no raw `launchArguments`, and no dirty relay/pubkey/event/secret-like fragments. The report must fail for any startup-network token hit, privacy forbidden fragment hit, selected Swift Testing suite with 0 tests, missing fixed result bundle path, missing selected suite counts, missing local gate report summary, non-collectionView route evidence, or dirty side-effect sentinel.

Continue to use `Documents/Plans/timeline_repository_db_adapter_adr.md`, `Documents/Plans/timeline_quote_materialization_adr.md`, and `Documents/Plans/timeline_repository_adapter_promotion_adr.md` as boundaries for any future real DB adapter or write-path work.
