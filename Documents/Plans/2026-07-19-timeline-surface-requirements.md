...
1. plain本文は軽量なSwiftUI `Text`、custom emoji本文はTextKitで描画する。いずれも表示前に
   font、line spacing、line limit、幅を確定し、非同期resource解決では本文geometryを変えない。
2. snapshot適用前にRow heightを一度だけ投影し、全Rowのprefix sumを持つ不変な
   `TimelineFeedStableLayout`を構築する。
3. 表示Cellの`preferredLayoutAttributesFitting`や表示後の`systemLayoutSizeFitting`では再採寸せず、
   layoutのframeをそのまま使う。表示中のCell frameを真の高さとしてprojectionへ書き戻さない。
4. Scroll session中は既存Rowのheightを固定し、geometry updateはsession終了後にまとめる。
5. 適用時はAnchor Rowとoffsetを保存し、同じ画面座標へ復元する。
6. profile、avatar、relative time、action state、画像decodeなどの非同期更新はRow frameを変更しない。
7. Cell再利用時はhosting contentとSwiftUI stateを破棄し、entryのgeometry identity単位で再生成する。
8. UIKit Cellとcontent viewを投影frameでclipし、隣接Rowへ描画が侵入しないことを境界条件にする。
9. `UICollectionViewCell`が専用`UIHostingController`を所有し、そのviewをcontent viewの四辺へ固定する。
   Row配置に`UIHostingConfiguration`の内部content view再利用を使わない。
10. Cell内hostingの`safeAreaRegions`は空にする。Timelineのsafe areaとChrome余白はcollection surfaceが
    一度だけ管理し、Row内部へwindow由来のsafe areaを二重適用しない。

`UIHostingConfiguration`の表示後 fitting 値は、現在のCell frameを入力として変化するためgeometryの
authorityにしない。短文Rowでも再計測のたびに高さが増えることを再現しており、この値で補正すると
過去方向ではRowが連鎖的に伸び、逆方向ではprefix sumが連続変化してスクロールが跳ねる。

実データではCellとhosting viewが`y = 484.3`でも、内部SwiftUI rootだけ`y = 443.0`となる
41.3 pointの原点ずれも再現した。専用hosting controllerと`safeAreaRegions = []`でRow rootを
Cellの`(0, 0)`へ固定し、同じ投稿のauthor/avatarが上端で欠けないことを実画面で確認する。
