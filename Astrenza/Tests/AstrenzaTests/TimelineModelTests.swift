...
struct TimelineModelTests {
    ...
    @Test("Timeline row height signatures ignore paint-only changes")
    func timelineRowHeightSignatureIgnoresPaintOnlyChanges() {
        let avatar = AvatarStyle(
            primary: .blue,
            secondary: .purple,
            symbolName: "person.fill"
        )
        let original = TimelinePost(
            id: "signature-post",
            authorName: "Unresolved Author",
            handle: "pending@example.test",
            avatar: avatar,
            body: "A stable body",
            createdAt: 100,
            replyCount: 0,
        ...
        let paintOnlyUpdate = TimelinePost(
            id: original.id,
            authorName: "Resolved Profile With A Different Name",
            handle: "resolved@example.test",
            avatar: AvatarStyle(
                primary: .orange,
                secondary: .pink,
                symbolName: "photo",
                imageURL: URL(string: "https://example.test/avatar.jpg")
            ),
            body: original.body,
        ...
        #expect(
            TimelineRowHeightSignature(post: original) ==
                TimelineRowHeightSignature(post: paintOnlyUpdate)
        )
    }

    @Test("Timeline row height signatures ignore content hidden by fixed warning height")
    func timelineRowHeightSignatureIgnoresFixedWarningContent() {
        let avatar = AvatarStyle(
            primary: .blue,
            secondary: .purple,
            symbolName: "person.fill"
        )
        func post(body: String, reason: String) -> TimelinePost {
            TimelinePost(
                id: "warning-signature-post",
                authorName: "Warning",
                handle: "warning@example.test",
                avatar: avatar,
                body: body,
                createdAt: 100,
                replyCount: 0,
                ...
                media: nil,
                context: nil,
                contentWarning: TimelineContentWarning(reason: reason)
            )
        }

        #expect(
            TimelineRowHeightSignature(
                post: post(body: "Short", reason: "First")
            ) == TimelineRowHeightSignature(
                post: post(
                    body: String(repeating: "Long hidden content ", count: 80),
                    reason: "A different fixed-height warning"
                )
            )
        )
    }

    @Test("Timeline row height signatures detect layout changes across insertion")
    func timelineRowHeightSignatureDetectsLayoutChangesAcrossInsertion() {
        let avatar = AvatarStyle(
            primary: .blue,
            secondary: .purple,
            symbolName: "person.fill"
        ...
        let oldPost = post(id: "common", body: "Short")
        let changedPost = post(
            id: oldPost.id,
            body: "A longer body arriving with a structural insertion"
        )
        let insertedPost = post(id: "inserted", body: "Inserted")

        #expect(
            TimelineContentHeightAnchorPlanner.changedPostIDs(
                oldEntries: [.post(oldPost)],
                newEntries: [.post(insertedPost), .post(changedPost)]
            ) == [oldPost.id]
        )
    }

    @Test("Timeline measured heights require exact content and layout context")
    func timelineMeasuredHeightRequiresExactSignatureAndContext() {
        let post = MockTimelineData.posts[0]
        ...
        let widerContext = TimelineRowMeasurementContext(
            containerWidth: 430,
            displayScale: 3,
            contentSizeCategory: "UICTContentSizeCategoryL",
            layoutDirection: "ltr",
        ...
        let didRecord = cache.recordMeasuredHeight(
            240,
            for: post,
            context: context
        )
        #expect(didRecord)
        #expect(cache.height(for: post, context: context) == 240)
        #expect(cache.height(for: post, context: widerContext) != 240)
        #expect(!cache.hasMeasurement(for: post, context: widerContext))
    }

    @Test("Timeline row measurement context normalizes subpixel width noise")
    func timelineRowMeasurementContextNormalizesSubpixelWidthNoise() {
        func context(_ width: CGFloat) -> TimelineRowMeasurementContext {
            TimelineRowMeasurementContext(
        ...
        #expect(context(390) == context(390.1))
        #expect(context(390) != context(390.2))
    }
