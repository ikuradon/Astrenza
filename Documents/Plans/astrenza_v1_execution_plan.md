...
Keep collection view route construction closed while preparing the future construction slice. The next implementation task that could open construction is titled `test: construct TimelineHome collectionView route behind flag`, and must first add RED-first `TimelineHomeCollectionViewRouteBehindFlagConstructionTests`.

That future task may only construct a collection view route description or read-only/offline `TimelineSurfaceDependencyContainer` dependency path plus a no-window/offscreen or non-rendered `TimelineSurface` or `TimelineCollectionViewController` behind explicit `--timeline-engine=collectionView` and readiness gates. It must keep the default rendered route legacy, keep `renderedRouteAfterConstruction == legacy`, keep `routeActivationAllowed == false`, record the route/construction artifact chain, and keep existing readiness / plan / harness / artifact-chain tests green. It must avoid production Home/root/splash replacement; avoid defaulting to collection view; avoid activation/render switching; avoid removing or bypassing `NostrHomeTimelineStore`; keep relay/network/resolver/DB write/read marker/`feed_read_state`/`feed_items.pending_new`/`resolve_jobs`/Root `dataSource.apply` scope closed; prove no extra `NostrHomeTimelineStore` construction in the collection view route construction path; pass startup-network grep with no matches; pass artifact privacy guard/self-test; and prove every selected app suite executes a non-zero Swift Testing count. If any gate fails, do not construct; record the blocked artifact and keep legacy. Route activation/rendering switch is later than construction and requires a separate explicit scope. Rollback and manual fallback remain legacy.
