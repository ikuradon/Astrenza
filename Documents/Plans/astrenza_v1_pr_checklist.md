...
- [ ] Collection view activation/render switching remains closed in the current docs-only slice: current behavior is legacy rendering plus the flagged offscreen/non-rendered construction result, debug override `.collectionView` cannot bypass the launch flag, and default legacy remains required.
...
- [ ] Route activation/rendering switch remains later than construction: the construction slice must be reviewed PASS, route artifact must show no side effects, offscreen smoke and restore harness must pass, the Root body decision snapshot must still report legacy rendered route unless activation scope is explicitly open, manual rollback and manual fallback must be `legacy`, and if any gate fails, do not construct; record the artifact and keep legacy.
- [ ] Future route activation implementation is titled `test: activate TimelineHome collectionView route behind flag`; activation means Root/Home may render the already constructed collection view path, is later than construction, requires explicit `--timeline-engine=collectionView`, requires all construction gates still clean, keeps default without the flag legacy, and keeps manual rollback legacy.
- [ ] Future route activation allowed scope is limited to switching the rendered route only behind explicit flag and readiness, using the already constructed flagged collection view path, keeping route diagnostics / construction artifact chain recording, preserving Root shell first paint, keeping the Timeline restore gate scoped to the Timeline area only, keeping `networkWaitedBeforeInteractiveScrollMS == 0`, and keeping `readMarkerChanged == false`.
- [ ] Future route activation forbidden scope includes defaulting to collection view, removing or bypassing `NostrHomeTimelineStore`, starting relay/network before first interactive scroll, DB write/read marker/`feed_read_state`/`pending_new` mutation, `dataSource.apply` from Root, splash/root shell behavior changes, schema/migration/GitHub Actions/dependency changes, and same-session double mutation between legacy and collection view paths.
- [ ] Before future route activation opens, required gates are PASS: flagged construction review, offscreen/no-window smoke, initial restore snapshot coordinator harness, artifact chain consumer, startup network grep with no matches, privacy guard, coordinator-only `dataSource.apply`, no extra `NostrHomeTimelineStore`, no DB write/read marker mutation, and a Root body decision snapshot that explicitly permits activation scope.
- [ ] Future route activation tests exist before implementation opens activation: `TimelineHomeCollectionViewRouteActivationTests`, `activation_requires_explicit_flag`, `activation_requires_all_construction_gates`, `default_legacy_rendering_remains_default`, `activation_does_not_start_network_before_interactive_scroll`, `activation_does_not_advance_read_marker`, `activation_does_not_write_db`, `activation_does_not_call_dataSourceApply_from_Root`, `activation_does_not_construct_extra_NostrHomeTimelineStore`, `activation_records_route_and_construction_artifacts`, `activation_uses_timeline_area_restore_gate_only`, `activation_keeps_root_shell_first_paint`, `activation_rollback_returns_to_legacy`, and `selected_swift_testing_suites_non_zero`.
