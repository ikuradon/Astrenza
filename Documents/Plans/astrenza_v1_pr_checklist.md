...
- [ ] Collection view route construction means Root/Home may construct a `TimelineSurface` or `TimelineCollectionViewController` dependency path behind explicit `--timeline-engine=collectionView` and readiness gates; it is not default rendering, not route activation, and not a rendering switch.
- [ ] Collection view route construction remains closed in the current docs-only slice: current behavior is legacy rendering plus observed collection view placeholder only, debug override `.collectionView` cannot bypass the launch flag, and default legacy remains required.
- [ ] Before route construction opens, required gates are listed and PASS: Root no-op preflight, route diagnostics sink injection, root decision snapshot, snapshot consumer, selected Swift Testing suites non-zero, startup network grep with no matches, `networkWaitedBeforeInteractiveScrollMS == 0`, `readMarkerChanged == false`, `requiresNetworkWork == false`, `requiresDBWrite == false`, coordinator-only `dataSource.apply`, no extra collectionView-path `NostrHomeTimelineStore` construction, no DB write/read marker mutation, artifact privacy guard, `TimelineCollectionViewControllerSmokeTests`, and `TimelineInitialRestoreSnapshotCoordinatorHarnessTests`.
- [ ] Future route construction tests exist before implementation opens construction: `TimelineHomeRouteConstructionReadinessTests`, `TimelineHomeCollectionViewRouteConstructionTests`, `collectionView_route_construction_requires_explicit_flag`, `collectionView_route_construction_requires_readiness`, `default_legacy_does_not_construct_collectionView`, `debug_override_collectionView_does_not_bypass_flag`, `collectionView_construction_is_offscreen_or_no_window`, `collectionView_construction_does_not_start_network`, `collectionView_construction_does_not_write_db`, `collectionView_construction_does_not_advance_read_marker`, `collectionView_construction_does_not_call_dataSourceApply`, `collectionView_construction_records_route_artifact`, `legacy_rendering_remains_default`, and `selected_swift_testing_suites_non_zero`.
- [ ] First route construction implementation, when explicitly opened, is titled `test: construct TimelineHome collectionView route behind flag` or equivalent; it may only construct read-only/offline dependencies and a `TimelineSurface` or `TimelineCollectionViewController` behind explicit flag/readiness while keeping `renderedRoute == legacy` unless a later activation task opens rendering.
- [ ] First route construction implementation forbids defaulting to collection view, replacing legacy Home rendering, removing or bypassing `NostrHomeTimelineStore`, starting relay/network, writing DB/read marker/`feed_read_state`/`feed_items.pending_new`/`resolve_jobs`, calling `dataSource.apply` from Root, changing splash/root shell behavior, route activation/render switch, GitHub Actions changes, SQL schema/migration changes, legacy SwiftUI Timeline implementation changes, and URLSession/relay/media resolver/OGP resolver/profile resolver/real `ResolveCoordinator` connections.
- [ ] Route activation/rendering switch remains later than construction: the construction slice must be reviewed PASS, route artifact must show no side effects, offscreen smoke and restore harness must pass, the Root body decision snapshot must still report legacy rendered route unless activation scope is explicitly open, and manual rollback must be `legacy`.
