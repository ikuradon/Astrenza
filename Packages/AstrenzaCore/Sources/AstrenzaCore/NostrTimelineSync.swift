...
public struct NostrTimelineSnapshot: Codable, Equatable {
...
public final class NostrTimelineCache {
    ...
    public func snapshot(accountID: String, timelineKey: String) -> NostrTimelineSnapshot? {
        guard let data = defaults.data(forKey: key(accountID: accountID, timelineKey: timelineKey)) else {
            return nil
        }
        return try? JSONDecoder().decode(NostrTimelineSnapshot.self, from: data)
    }
    ...
    private func key(accountID: String, timelineKey: String) -> String {
        "nostr.timeline.snapshot.\(accountID).\(timelineKey)"
    }
