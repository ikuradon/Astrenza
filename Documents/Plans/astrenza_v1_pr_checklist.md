...
- [x] TimelineHome collectionView flagged startup/restore/scroll local gate complete.
- [x] Completed TimelineHome collectionView flagged local gate evidence keeps default/no flag legacy, requires explicit `--timeline-engine=collectionView`, requires a clean evaluated Root body wiring gate, has startup smoke PASS, visible restored rows PASS, restore scroll position PASS, no startup network, no DB write, no read marker mutation, no `pending_new` mutation, no Root `dataSource.apply`, no extra `NostrHomeTimelineStore`, and selected Swift Testing suites non-zero.
- [ ] Next TimelineHome collectionView phase chooses only one of: actual data display quality, scroll interaction behavior, local pagination/windowing, manual visual smoke, or release/default decision.
